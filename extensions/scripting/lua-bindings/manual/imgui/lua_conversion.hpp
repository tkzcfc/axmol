...
namespace imlua
{
	inline void ref_type_to_luaval(lua_State* L, ax::Ref* ref, const char* typeID)
    {
        if (ref)
        {
            ...
            std::string luatype = "ax.Ref";
            if (g_luaType.end() != iter)
				luatype = iter->second;
            toluafix_pushusertype_object(
	...
	template<typename T>
	struct to_native<T*, std::enable_if_t<std::is_base_of_v<ax::Ref, T>>> {
		static bool F(lua_State* L, int lo, T** outValue, const char* fName = "") {
			...
			if (className.empty()) className = "ax.Ref";
	...
	template<>
	struct to_native<ax::Vec2> {
		static bool F(lua_State* L, int lo, ax::Vec2* outValue, const char* fName = "") {
			LUA_CHECK_TO_NATIVE;
			if (!is_indexable(L, lo)) return false;
			ax::Vec2 v;
			bool ok = true;
	...
	LUA_TO_NATIVE_STRUCT3(ax::Vec3, x, y, z);
	LUA_TO_NATIVE_STRUCT4(ax::Vec4, x, y, z, w);
	LUA_TO_NATIVE_STRUCT2(ax::BlendFunc, src, dst);
	LUA_TO_NATIVE_STRUCT3(ax::Color3B, r, g, b);
	LUA_TO_NATIVE_STRUCT4(ax::Color4B, r, g, b, a);
	LUA_TO_NATIVE_STRUCT4(ax::Color4F, r, g, b, a);
	LUA_TO_NATIVE_STRUCT4(ax::Quaternion, x, y, z, w);
	LUA_TO_NATIVE_STRUCT2(ax::MeshVertexAttrib, type, vertexAttrib);
	LUA_TO_NATIVE_STRUCT3(ax::V3F_C4B_T2F, vertices, colors, texCoords);
	LUA_TO_NATIVE_STRUCT4(ax::V3F_C4B_T2F_Quad, tl, tr, bl, br);
	LUA_TO_NATIVE_STRUCT3(ax::backend::AttributeBindInfo, location, size, type);

#if AX_USE_PHYSICS
	LUA_TO_NATIVE_STRUCT3(ax::PhysicsMaterial, density, restitution, friction);

	template<>
	struct to_native<ax::PhysicsContactData> {
		static bool F(lua_State* L, int lo, ax::PhysicsContactData* outValue, const char* fName = "") {
			LUA_CHECK_TO_NATIVE;
			if (!is_indexable(L, lo)) return false;
			ax::PhysicsContactData v;
	...
	template<>
	struct to_native<ax::PhysicsRayCastInfo> {
		static bool F(lua_State* L, int lo, ax::PhysicsRayCastInfo* outValue, const char* fName = "") {
			LUA_CHECK_TO_NATIVE;
			if (!is_indexable(L, lo)) return false;
			ax::PhysicsRayCastInfo v{};
	...
	template<>
	struct to_native<ax::Rect> {
		static bool F(lua_State* L, int lo, ax::Rect* outValue, const char* fName = "") {
			LUA_CHECK_TO_NATIVE;
			if (!is_indexable(L, lo)) return false;
			ax::Rect v;
	...
	template<>
	struct to_native<ax::AffineTransform> {
		static bool F(lua_State* L, int lo, ax::AffineTransform* outValue, const char* fName = "") {
			LUA_CHECK_TO_NATIVE;
			if (!lua_istable(L, lo)) return false;
			ax::AffineTransform v{};
	...
	template<>
	struct to_native<ax::FontDefinition> {
		static bool F(lua_State* L, int lo, ax::FontDefinition* outValue, const char* fName = "") {
			LUA_CHECK_TO_NATIVE;
			if (!lua_istable(L, lo)) return false;
			ax::FontDefinition v;
	...
	template<>
	struct to_native<ax::TTFConfig> {
		static bool F(lua_State* L, int lo, ax::TTFConfig* outValue, const char* fName = "") {
			LUA_CHECK_TO_NATIVE;
			if (!lua_istable(L, lo)) return false;
			ax::TTFConfig v;
	...
	template<>
	struct to_native<ax::Value> {
		static bool F(lua_State* L, int lo, ax::Value* outValue, const char* fName = "") {
			LUA_CHECK_TO_NATIVE;
			...
			if (is_indexable(L, lo))
			{
				...
				if (lua_isnil(L, -1)) /** if table[1] = nil,we don't think it is a pure array */
				{
					...
					if ((ok = luaval_to_native(L, lo, &dictVal, fName)))
						*outValue = ax::Value(dictVal);
				}
				else
				{
					lua_pop(L, 1);
					ax::ValueVector arrVal;
					if ((ok = luaval_to_native(L, lo, &arrVal, fName)))
						*outValue = ax::Value(arrVal);
				}
			}
			else if (type == LUA_TSTRING)
			{
				...
				if ((ok = luaval_to_native(L, lo, &stringValue, fName)))
					*outValue = ax::Value(stringValue);
			}
			else if (type == LUA_TBOOLEAN)
			{
				bool boolVal = false;
				if ((ok = luaval_to_native(L, lo, &boolVal, fName)))
					*outValue = ax::Value(boolVal);
			}
			else if (type == LUA_TNUMBER)
			{
				double doubleVal = 0;
				if ((ok = luaval_to_native(L, lo, &doubleVal, fName)))
					*outValue = ax::Value(doubleVal);
			}
			else if (type == LUA_TNIL)
			{
				*outValue = ax::Value::Null;
			}
			return ok;
		}
	};
	...
	template<>
	struct to_native<ax::Mat4> {
		static bool F(lua_State* L, int lo, ax::Mat4* outValue, const char* fName = "") {
			LUA_CHECK_TO_NATIVE;
			if (!is_indexable(L, lo)) return false;
	...
	template<>
	struct to_native<ax::Texture2D::TexParams> {
		static bool F(lua_State* L, int lo, ax::Texture2D::TexParams* outValue, const char* fName = "") {
			LUA_CHECK_TO_NATIVE;
			if (!is_indexable(L, lo)) return false;
			ax::Texture2D::TexParams v;
	...
	//template<>
	//struct to_native<ax::backend::UniformLocation> {
	//	static bool F(lua_State* L, int lo, ax::backend::UniformLocation* outValue, const char* fName = "") {
	//		LUA_CHECK_TO_NATIVE;
	//		if (!is_indexable(L, lo)) return false;
	//		ax::backend::UniformLocation v;
	...
	template<>
	struct to_native<ax::backend::BlendDescriptor> {
		static bool F(lua_State* L, int lo, ax::backend::BlendDescriptor* outValue, const char* fName = "") {
			LUA_CHECK_TO_NATIVE;
			if (!is_indexable(L, lo)) return false;
			ax::backend::BlendDescriptor v;
	...
	template<typename T>
	struct to_lua<T*, std::enable_if_t<std::is_base_of_v<ax::Ref, T>>> {
		static void F(lua_State* L, T* inValue) {
			if (!L) return;
			ref_type_to_luaval(L, (ax::Ref*)inValue, typeid(T).name());
		}
	};
	...
	LUA_TO_LUA_STRUCT2(ax::Vec2, x, y);
	LUA_TO_LUA_STRUCT3(ax::Vec3, x, y, z);
	LUA_TO_LUA_STRUCT4(ax::Vec4, x, y, z, w);
	LUA_TO_LUA_STRUCT2(ax::BlendFunc, src, dst);
	LUA_TO_LUA_STRUCT3(ax::Color3B, r, g, b);
	LUA_TO_LUA_STRUCT4(ax::Color4B, r, g, b, a);
	LUA_TO_LUA_STRUCT4(ax::Color4F, r, g, b, a);
	LUA_TO_LUA_STRUCT4(ax::Quaternion, x, y, z, w);
	LUA_TO_LUA_STRUCT2(ax::MeshVertexAttrib, type, vertexAttrib);
	LUA_TO_LUA_STRUCT3(ax::V3F_C4B_T2F, vertices, colors, texCoords);
	LUA_TO_LUA_STRUCT4(ax::V3F_C4B_T2F_Quad, tl, tr, bl, br);
	LUA_TO_LUA_STRUCT6(ax::AffineTransform, a, b, c, d, tx, ty);
	LUA_TO_LUA_STRUCT6(ax::TTFConfig, fontFilePath, fontSize, glyphs, customGlyphs, distanceFieldEnabled, outlineSize);
	LUA_TO_LUA_STRUCT4(ax::Texture2D::TexParams, minFilter, magFilter, sAddressMode, tAddressMode); // ax::backend::SamplerDescriptor
	LUA_TO_LUA_STRUCT3(ax::backend::AttributeBindInfo, location, size, type);

#if AX_USE_PHYSICS
	LUA_TO_LUA_STRUCT3(ax::PhysicsMaterial, density, restitution, friction);

	template<>
	struct to_lua<ax::PhysicsContactData> {
		static void F(lua_State* L, const ax::PhysicsContactData& inValue) {
			if (!L) return;
	...
	template<>
	struct to_lua<ax::PhysicsRayCastInfo> {
		static void F(lua_State* L, const ax::PhysicsRayCastInfo& inValue) {
			if (!L) return;
	...
	template<>
	struct to_lua<ax::Rect> {
		static void F(lua_State* L, const ax::Rect& inValue) {
			if (!L) return;
	...
	template<>
	struct to_lua<ax::FontDefinition> {
		static void F(lua_State* L, const ax::FontDefinition& inValue) {
			if (!L) return;
	...
	template<>
	struct to_lua<ax::Mat4> {
		static void F(lua_State* L, const ax::Mat4& inValue) {
			if (!L) return;
	...
	template<>
	struct to_lua<ax::Value> {
		static void F(lua_State* L, const ax::Value& inValue) {
			if (!L) return;
	...
	//template<>
	//struct to_lua<ax::backend::UniformLocation> {
	//	static void F(lua_State* L, const ax::backend::UniformLocation& inValue) {
	//		if (!L) return;
	...
	template<>
	struct to_lua<ax::backend::BlendDescriptor> {
		static void F(lua_State* L, const ax::backend::BlendDescriptor& inValue) {
			if (!L) return;
			lua_createtable(L, 0, 4);
...
#define LUA_TRY_INVOKE(ARGC_, Fn_)\
	if (argc == (ARGC_) && !ok) {\
		...
		lua_insert(lua_S, 1);\
		ok = imlua::invoke_native(lua_S, arg_cur, (Fn_));\
		if (ok) {\
			lua_settop(lua_S, 1);\
			return 1;\
		} else {\
...
#define LUA_CLS_END()\
	...
	imlua::registerLuaType<current_type>(typeName, castName);
#endif // !LUA_CLS_END
...
#define LUA_PUSH_NATIVE(V_)\
	imlua::native_to_luaval(lua_S, V_);
#endif // !LUA_PUSH_NATIVE

#ifndef LUA_NATIVE_SETTER
#define LUA_NATIVE_SETTER(V_, IDX_)\
	if (!imlua::luaval_to_native(lua_S, (IDX_), &(V_), LUA_FNAME))\

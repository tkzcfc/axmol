...
static int imgui_image(lua_State *L) {
	const int args = lua_gettop(L);
	ax::Texture2D* tex = nullptr;
	if (luaval_to_object(L, 1, "ax.Texture2D", &tex))
	{
		if (!tex)
			return 0;
        ax::extension::ImGuiPresenter::getInstance()->image(tex,
	...
	if (luaval_to_object(L, 1, "ax.Sprite", &sp))
	{
		if (!sp)
			return 0;
		ax::extension::ImGuiPresenter::getInstance()->image(sp,
			lua_opt_imv2(args, 2, ImVec2(0, 0)),
            lua_opt_bool(args, 3, true),
			lua_opt_imv4(args, 4, ImVec4(1, 1, 1, 1)),
			lua_opt_imv4(args, 5, ImVec4(0, 0, 0, 0)));
		return 0;
	}
    ax::SpriteFrame* frame = nullptr;
    if (luaval_to_object(L, 1, "ax.SpriteFrame", &frame))
    {
        if (!frame)
            return 0;
        ax::extension::ImGuiPresenter::getInstance()->image(frame,
            lua_opt_imv2(args, 2, ImVec2(0, 0)),
            lua_opt_bool(args, 3, true),
            lua_opt_imv4(args, 4, ImVec4(1, 1, 1, 1)),
            lua_opt_imv4(args, 5, ImVec4(0, 0, 0, 0)));
...
//			return 0;
//		lua_pushboolean(L,
//			ax::extension::ImGuiPresenter::getInstance()->imageButton(tex,
//				lua_opt_imv2(args, 2, ImVec2(0, 0)),
//				lua_opt_imv2(args, 3, ImVec2(0, 0)),
//				lua_opt_imv2(args, 4, ImVec2(1, 1)),
...
//			return 0;
//		lua_pushboolean(L,
//			ax::extension::ImGuiPresenter::getInstance()->imageButton(sp,
//				lua_opt_imv2(args, 2, ImVec2(0, 0)),
...
// Drag and Drop TODO: release lua values in ax::extension::ImGuiPresenterLayer?
...
//		if (arr.size() % 2 == 0)
//			arr.push_back(0);
//		ranges = ax::extension::ImGuiPresenter::getInstance()->addGlyphRanges(storeKey, arr);
//	}
//	else
//	{
...
static int imgui_launch(lua_State* L)
{
...
static int imgui_clearFonts(lua_State* L)
{
    ax::extension::ImGuiPresenter::getInstance()->clearFonts();
    return 0;
}
static int imgui_removeFont(lua_State* L)
{
    std::string path;
    if (!luaval_to_std_string(L, 1, &path))
        return 0;
    ax::extension::ImGuiPresenter::getInstance()->removeFont(path);
    return 0;
}
static int imgui_addFont(lua_State* L)
{
    std::string path;
    if (!luaval_to_std_string(L, 1, &path))
        return 0;
    const int args = lua_gettop(L);
    ax::extension::ImGuiPresenter::getInstance()->addFont(path, lua_opt_int(args, 2, 13));
    return 0;
}
...
static int imgui_ccNode(lua_State *L) {
	...
	if (luaval_to_object(L, 1, "ax.Node", &node))
	{
		if (!node)
			return 0;
		ax::extension::ImGuiPresenter::getInstance()->node(node,
...
//			return 0;
//		lua_pushboolean(L,
//			ax::extension::ImGuiPresenter::getInstance()->nodeButton(node,
...
static int imgui_setCCNodeColor(lua_State *L)
{
	...
	if (luaval_to_object(L, 1, "ax.Node", &node))
	{
		...
		if (type == LUA_TNUMBER)
		{
			...
			ax::extension::ImGuiPresenter::setNodeColor(node, idx);
		}
		else if (type == LUA_TTABLE)
		{
			ax::extension::ImGuiPresenter::setNodeColor(node, _luaval_to_imvec4(L, 2));
		}
	}
	return 0;
}
static int imgui_setCCLabelColor(lua_State *L)
{
	...
	if (luaval_to_object(L, 1, "ax.Label", &label))
	{
		if (!label) return 0;
		const auto type = lua_type(L, 2);
		if (type == LUA_TNUMBER)
		{
			...
			if (idx < 0 || idx >= ImGuiCol_COUNT)
				return luaL_error(L, "invalid parameter #2");
			ax::extension::ImGuiPresenter::setLabelColor(label, (ImGuiCol)idx);
		}
		else if (type == LUA_TTABLE)
			ax::extension::ImGuiPresenter::setLabelColor(label, _luaval_to_imvec4(L, 2));
		else
			ax::extension::ImGuiPresenter::setLabelColor(label, lua_toboolean(L, 2) != 0);
	}
	return 0;
}
...
//	if (lua_isnil(L, 1))
//	{
//		ax::extension::ImGuiPresenter::getInstance()->setMarkdownLinkCallback(nullptr);
//		return 0;
//	}
...
//		lua_call(L, 3, 0);
//	};
//	ax::extension::ImGuiPresenter::getInstance()->setMarkdownLinkCallback(f);
//	return 0;
//}
...
//	if (lua_isnil(L, 1))
//	{
//		ax::extension::ImGuiPresenter::getInstance()->setMarkdownTooltipCallback(nullptr);
//		return 0;
//	}
//	luaL_checktype(L, 1, LUA_TFUNCTION);
...
//		lua_call(L, 4, 0);
//	};
//	ax::extension::ImGuiPresenter::getInstance()->setMarkdownTooltipCallback(f);
//	return 0;
//}
...
//		// ... f, r1, r2, r3, r4
//		ax::Sprite* sp = nullptr;
//		luaval_to_object(L, -4, "ax.Sprite", &sp);
//		ImVec2 size(0, 0);
...
//	};
//	ax::extension::ImGuiPresenter::getInstance()->setMarkdownImageCallback(f);
//	return 0;
//}
...
//	if (lua_isnil(L, 1))
//	{
//		ax::extension::ImGuiPresenter::getInstance()->setMarkdownFormatCallback(nullptr);
//		return 0;
//	}
//	luaL_checktype(L, 1, LUA_TFUNCTION);
...
//		lua_call(L, 4, 0);
//	};
//	ax::extension::ImGuiPresenter::getInstance()->setMarkdownFormatCallback(f);
//	return 0;
//}
...
//	ax::extension::ImGuiPresenter::getInstance()->setMarkdownFont(index, font, lua_toboolean(L, 3));
//	return 0;
//}
...
//	ax::extension::ImGuiPresenter::getInstance()->setMarkdownLinkIcon(std::string(s, size));
//	return 0;
//}
...
//	size_t size;
//	const auto s = luaL_checklstring(L, 1, &size);
//	ax::extension::ImGuiPresenter::getInstance()->markdown(std::string(s, size));
//	return 0;
//}
...
static const luaL_Reg imgui_methods[] = {
	...
	// cocos
    M(clearFonts), M(removeFont), M(addFont),
	M(ccNode),
    //M(ccNodeButton),
...
static int drawlist_addCCNode(lua_State *L)
{
	...
	int id;
	ax::Node* node = nullptr;
	if (luaval_to_object(L, 2, "ax.Node", &node))
	{
		if (!node) return 0;
		std::tie(tid, pmin, pmax, id) = ax::extension::ImGuiPresenter::getInstance()->useNode(node, _luaval_to_imvec2(L, 3));
		if (!tid) return 0;
		cobj->AddImage(
			tid,
			pmin,
...
static int drawlist_addImage(lua_State *L)
{
	...
	int id;
	ax::Texture2D* tex = nullptr;
	if (luaval_to_object(L, 2, "ax.Texture2D", &tex))
	{
		if (!tex) return 0;
		std::tie(tid, id) = ax::extension::ImGuiPresenter::getInstance()->useTexture(tex);
		if (!tid) return 0;
		cobj->AddImage(
			tid,
	...
	ax::Sprite* sp = nullptr;
	if (luaval_to_object(L, 2, "ax.Sprite", &sp))
	{
		if (!sp) return 0;
		std::tie(tid, uva, uvb, id) = ax::extension::ImGuiPresenter::getInstance()->useSprite(sp);
		if (!tid) return 0;
		cobj->AddImage(
			tid,
			_luaval_to_imvec2(L, 3),
...
static int drawlist_addImageQuad(lua_State *L)
{
	...
	{
		if (!sp) return 0;
		std::tie(tid, uva, uvc, id) = ax::extension::ImGuiPresenter::getInstance()->useSprite(sp);
		if (!tid) return 0;
		cobj->AddImageQuad(
			tid,
			_luaval_to_imvec2(L, 3),
...
//			return 0;
//		}
//		ax::extension::ImGuiPresenter::mergeFontGlyphs(cobj, arg0,
...
//		if (!tex)
//			return 0;
//		ax::extension::ImGuiPresenter::getInstance()->plotImage(
//			luaL_checkstring(L, 1),
//			tex,
//			_luaval_to_imvec2(L, 3),
//			_luaval_to_imvec2(L, 4),
...
int luaopen_imgui(lua_State *L)
{
	...
	ax::extension::ImGuiPresenter::setOnInit([=](ax::extension::ImGuiPresenter* ins)
	{
        ins->addRenderLoop("__luadraw__" , [=]()
		{
